...
public class Heap<E> extends AbstractQueue<E> {
	...
	public E poll() {
		...
		int parent = 0; 
		int childLeft = getLeft(parent); 
		int childRight = getRight(parent); 
		while ((childLeft < theData.size() && compare(parent, childLeft) > 0)
				|| (childRight < theData.size() && compare(parent, childRight) > 0)) {
			if (childRight < theData.size() && compare(childRight, childLeft) <= 0) {
				swap(parent, childRight);
				parent = childRight;
				childLeft = getLeft(parent);
				childRight = getRight(parent);
			} else {
	...
	public boolean remove(Object o) {
		...
		int childLeft = getLeft(parent);
		int childRight = getRight(parent);
		while (!isRoot(parent) && (compare(parent, childLeft) > 0 || compare(parent,childRight) > 0)) {
			if(compare(childRight,childLeft) < 0) { 
				swap(parent,childRight); 
				parent = getParent(childRight); 
				childLeft = getLeft(parent); 
				childRight = getRight(parent); 
			} else { 
				swap(parent,childLeft); 
				parent = getParent(childLeft); 
				childLeft = getLeft(parent); 
				childRight = getRight(parent); 
